...
#JMR LLM Nov 3rd added Adafactor optimizer and MobileLLM optimizations matching paperwould you 
import os
import torch
...
# Add this with other global configurations
use_mobile_optimizations = True  # Set this flag to switch between versions
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        if self.flash_available:
            ...
            # Flash attention with correct mask
            ...
            # Original attention path (keep as fallback)
...
class TransformerModel(nn.Module):
    def __init__(self, vocab_size, n_embd, n_head, block_size, n_layer, dropout, pretrained_embeddings=None, use_chess=False):
        super().__init__()
    ...
    def forward(self, idx, targets=None):
        ...
        game_mask = self.create_game_mask(idx) if self.use_chess else None
...
def save_model_all(model, all_text, n_embd, n_head, n_kv_heads, n_layer, dropout, block_size, epoch, batch_idx, optimizer, scheduler, scaler, loss):
    ...
    checkpoint = {
        ...
        'hyperparameters': {
            ...
            'use_chess': use_chess  # Always save it - it's either True or False
        },
    }
...
def load_model_file(vocab_size, n_embd, n_head, block_size, n_layer, dropout):
    ...
    if model_file:
        ...
        # Add default n_kv_heads if not present in older models
        n_kv_heads = hyperparameters.get('n_kv_heads', n_head // 3)  # Default to 1/3 of query heads
        
        # Get the dataset type and tokenizer - with fallback for legacy models
        dataset_type = checkpoint.get('dataset_type', 'text')  # Default to 'text' for legacy
        tokenizer = checkpoint.get('tokenizer')

        # Determine use_chess from either:
        # 1. Explicit hyperparameter (new models)
        # 2. Dataset type (current models)
        # 3. Presence of chess-specific tokenizer (legacy models)
        use_chess = (
            hyperparameters.get('use_chess', None) or  # Try new explicit parameter first
            dataset_type in ['chess_moves', 'chess_characters'] or  # Try current method
            isinstance(tokenizer, dict) and '<STARTGAME>' in tokenizer  # Legacy detection
        )
        ...
        print(f"Using chess mode: {use_chess}")

        # Only get n_kv_heads if using mobile optimizations
        if use_mobile_optimizations:
            # For new models with explicit KV heads
            if 'n_kv_heads' in hyperparameters:
                n_kv_heads = hyperparameters['n_kv_heads']
            else:
                # For loading old models into new architecture
                print("Old model detected, using default 3 KV heads for 9:3 ratio")
                n_kv_heads = 3  # Your preferred ratio for chess
            ...
            # Original TransformerModel without KV heads
        ...
        return (model, vocab_size, n_embd, n_head, n_kv_heads, block_size, n_layer, dropout, 
                tokenizer, use_chess, move_to_token, dataset_type, optimizer_state_dict, 
                scheduler_state_dict, scaler_state_dict, last_epoch, last_batch_idx)
    else:
        print("No model file selected. Creating a new model...")
        return None


def specify_model_parameters(vocab_size, n_embd, n_head, block_size, n_layer, dropout, use_gpt2=False):
    """
    Specifies model parameters with support for:
    - Chess moves tokenization (custom vocab)
    ...
    # Handle GPT2 if requested
    if use_gpt2:
        use_pretrained_embeddings = bool(int(get_input_with_default("Use pretrained GPT2 embeddings? (1/0)", 0)))
    else:
        use_pretrained_embeddings = False

    # Get number of query heads
    n_head = int(get_input_with_default("Enter number of query heads (n_head)", 9))  # Default to 9
    n_kv_heads = int(get_input_with_default("Enter number of KV heads (recommended: 3 for 9:3 ratio)", 3))  # Default to 3

    # Get embedding dimensions
    n_embd = int(get_input_with_default("Enter embedding dimensions (n_embd)", n_embd))
    
    # Ensure dimensions work with both head types
    if n_embd % n_head != 0 or n_embd % n_kv_heads != 0:
        print(f"Warning: n_embd ({n_embd}) should be divisible by both:")
        print(f"- n_head ({n_head}) for query heads")
        print(f"- n_kv_heads ({n_kv_heads}) for key/value heads")
        return None

    # Rest of parameters
    block_size = int(get_input_with_default("Enter seq_length/block_size", block_size))
    ...
    print(f"- Query heads: {n_head} (dim per head: {n_embd//n_head})")
    print(f"- Key/Value heads: {n_kv_heads} (dim per head: {n_embd//n_kv_heads})")
    print(f"- KV sharing ratio: {n_head//n_kv_heads}:1")
    print(f"- Sequence length: {block_size}")
    print(f"- Layers: {n_layer}")
    print(f"- Dropout: {dropout}")
    print(f"- Using GPT2: {use_gpt2}")
    ...
    if use_mobile_optimizations:
        model = MobileLLMModel(
            ...
            n_embd=n_embd,
            n_head=n_head,
            n_kv_heads=n_kv_heads,  # Pass explicit KV heads
            block_size=block_size,
            n_layer=n_layer,
            dropout=dropout,
            use_chess=use_chess
    ...
    return model, vocab_size, n_embd, n_head, n_kv_heads, block_size, n_layer, dropout, use_pretrained_embeddings
...
def test_progress(epoch, num_epochs, batch_idx, data_loader, loss, model, x, tokens_to_generate, all_text, tokenizer_or_idx_to_char_or_move):
    ...
    with torch.no_grad():
        ...
        if isinstance(tokenizer_or_idx_to_char_or_move, dict):  # For character-based tokenizer
            if len(tokenizer_or_idx_to_char_or_move) > 256:  # Assuming move-based tokenization has more than 256 entries   
                generated_text = ' '.join([tokenizer_or_idx_to_char_or_move.get(idx, '<PAD>') for idx in generated_tokens])
            else:  # Character-based tokenization
...
if __name__ == "__main__":
    ...
    model_choice = input("Choose model version (M: Mobile, S: Standard): ").lower()
    if model_choice == "m": 
        use_mobile_optimizations = True  # Set this flag to switch between versions
    else:
        use_mobile_optimizations = False
    ...
    try:
        ...
        if load_or_create == 'l':
            ...
            if loaded_model:
                model, vocab_size, n_embd, n_head, n_kv_heads, block_size, n_layer, dropout, tokenizer, use_chess, move_to_token, dataset_type, optimizer_state_dict, scheduler_state_dict, scaler_state_dict, last_epoch, last_batch_idx = (*loaded_model, 3)[:17]
        ...
        model, vocab_size, n_embd, n_head, n_kv_heads, block_size, n_layer, dropout, use_pretrained_embeddings = specify_model_parameters(
            vocab_size=vocab_size, 
            n_embd=n_embd, 
            n_head=n_head, 
            block_size=block_size, 
    ...
    if optimizer_state_dict:
        ...
        if use_mobile_optimizations:
            #let the user enter new learning rate and weight decay use defulats from they hyperparmaters like below
            # del that we just loaded
            ...
            clip_threshold = float(get_input_with_default(f"Enter clip threshold (default:1)", 1))
            beta1 = float(get_input_with_default(f"Enter beta1 (default: 0.9)", 0.9)) 

            # Create Adafactor with same settings
            optimizer = Adafactor(
                model.parameters(),
                lr=learning_rate,
        ...
        # Initialize the state before loading
            for group in optimizer.param_groups:
                for p in group['params']:
                    state = optimizer.state[p]
                    # Initialize required state elements if they don't exist
            ...
            optimizer.load_state_dict(optimizer_state_dict)
             # Add the quick fix right after loading
            for group in optimizer.param_groups:
                if 'beta1' not in group:
                    group['beta1'] = 0.9
            print("Loaded Adafactor state from checkpoint")
        ...
        if use_mobile_optimizations:
            # MobileLLM paper recommendations with user input
            learning_rate = float(get_input_with_default("Enter learning rate (MobileLLM default: 6e-4)", 6e-4))
            weight_decay = float(get_input_with_default("Enter weight decay (MobileLLM default: 0.1)", 0.1))
            beta1 = float(get_input_with_default("Enter beta1 (MobileLLM default: 0.9)", 0.9))
            clip_threshold = float(get_input_with_default("Enter clip threshold (MobileLLM default: 1.0)", 1.0))
            #beta2 = float(get_input_with_default("Enter beta2 (MobileLLM default: 0.95)", 0.95))
        ...
        if use_pretrained_embeddings and not use_mobile_optimizations:
    ...
    if not use_mobile_optimizations:
        max_norm = float(get_input_with_default("Enter max norm (default: 1.0)", 1.0))
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for batch_idx, (x, y) in enumerate(data_loader):
            ...
            if device.type == 'cuda':
                ...
                if not use_mobile_optimizations:  # Only clip for AdamW
                ...
                else:  # For MobileLLM with Adafactor, clipping is handled internally
                    # Optionally monitor gradient norms without clipping
                    
                    total_norm = torch.nn.utils.clip_grad_norm_(model.parameters(), float('inf'))
                    
                    if total_norm > clip_threshold:
                        print(f"Adafactor clipping triggered at batch {batch_idx} in epoch {epoch}. Total norm: {total_norm:.2f}. Loss: {loss:.4f}")
                ...
                if not use_mobile_optimizations:
                    ...
                    # For MobileLLM with Adafactor, clipping is handled internally
                    total_norm = torch.nn.utils.clip_grad_norm_(model.parameters(), float('inf'))
                    if total_norm > clip_threshold:
                        print(f"Adafactor clipping triggered at batch {batch_idx} in epoch {epoch}. Total norm: {total_norm:.2f}. Loss: {loss:.4f}")
                optimizer.step()
            ...
            if (batch_idx + 1) % inference_frequency == 0:
                ...
                if use_mobile_optimizations:
                    with torch.no_grad():  # Completely safe - won't affect training
                        try:
                            logits = output[0]  # Get logits from MobileLLM output
                            B, T, C = logits.shape
                            monitored_loss = F.cross_entropy(logits.view(B*T, C), y.view(B*T))
                            print(f"\nMonitored Loss: {monitored_loss.item():.4f}")
                            loss = monitored_loss
                        except Exception as e:
                               print(f"Loss monitoring failed (training continues): {e}")
                ...
                # Save the model and generated text
                save_model_all(model, all_text, n_embd, n_head, n_kv_heads, n_layer, dropout, block_size, epoch, start_batch +batch_idx, optimizer, scheduler, scaler,loss)
